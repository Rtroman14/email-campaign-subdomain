...
module.exports = class HighlevelApi {
    ...
    async addToWorkflow(contactID, workflowId) {
        try {
            ...
            console.log("addToWorkflow ---", error);
            return false;
        }
    }

    async outreachContact(contactData, workflowId) {
        try {
            const contact = await this.createContact(contactData);

            if (contact) {
                const res = await this.addToWorkflow(contact.id, workflowId);
            ...
            console.log("outreachContact() ---", error);
            return false;
        }
    }
